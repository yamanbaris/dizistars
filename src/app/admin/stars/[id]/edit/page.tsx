...
import { getStar, updateStar } from '@/lib/database';
...
export default function EditStarPage() {
  ...
  useEffect(() => {
    async function loadStar() {
      try {
        const star = await getStar(id as string);
        if (star) {
          setFormData({
            ...star,
            current_project: star.current_project || '',
            social_media_handles: {
              instagram: star.social_media_handles?.instagram || '',
              twitter: star.social_media_handles?.twitter || '',
              tiktok: star.social_media_handles?.tiktok || ''
            }
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id) {
      console.error('No star ID provided');
      toast.error('Invalid star ID');
      return;
    }
    ...
    try {
      ...
      const submissionData = {
        ...
        notable_works: formData.notable_works || [],
        filmography: formData.filmography || [],
        gallery_images: formData.gallery_images || [],
        is_featured: formData.is_featured,
        is_trending: formData.is_trending,
        is_rising: formData.is_rising,
        is_influential: formData.is_influential
      };

      console.log('Submission data prepared:', submissionData);
      console.log('Attempting to update star with ID:', id);

      try {
        const result = await updateStar(id as string, submissionData);
        ...
        if (result) {
          ...
          console.error('No result returned from updateStar');
          throw new Error('Failed to update star - no result returned');
        }
      } catch (updateError) {
        console.error('Error in updateStar:', updateError);
        toast.error(updateError instanceof Error ? updateError.message : 'Failed to update star');
        throw updateError;
      }
    } catch (error) {
      ...
      <div className="sticky top-0 z-50 bg-gray-800/80 backdrop-blur-sm border-b border-gray-700">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-semibold text-white">Edit Star Profile</h1>
          </div>
          <div className="flex items-center space-x-4">
            <button
              type="button"
      ...
      {loading ? (
        <div className="flex items-center justify-center min-h-[calc(100vh-4rem)]">
          <div className="flex flex-col items-center space-y-4">
            ...
            <p className="text-gray-400">Loading star data...</p>
          </div>
        </div>
      ) : (
              ...
              >
                <form id="star-form" onSubmit={handleSubmit} className="space-y-8">
                  <div className="bg-gray-800 rounded-lg shadow-lg">
                    {/* Keep existing tab content, but wrap each field group in a card */}
